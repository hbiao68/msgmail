...
<form name="form1" action="${pageContext.request.contextPath}/plugin/pluginAddAction.do" method="post" enctype="multipart/form-data" onsubmit="return fileCheckOnSubmit();">

		
		<div class="bd">
				<div class="biginfo">添加插件</div>
				<div class="tpinfo">
					...
					<div style="margin:10px 0 8px 61px">
						...
						<select name="importformat" id="importformat">
							<option value="csv">jar文件(*.jar)</option>
...
function fileCheck(){
	...
	if(filetext==''){
		...
		if(result.toString().toLowerCase() == '.jar' ){	//toString不可省去
			return true;
		}else{
			//alert("请选择CSV格式文件导入");
			var msg = '请选择jar格式文件导入';
			span.innerHTML = msg;
			span.style.color='#ff0000';
		    span.style.fontSize='12px';
			return false;
