...
<body onload="init()">
    <div style="display:block;width:100%;min-width:1122px;height:300px;">
        <form action="<%=request.getContextPath()%>/relation/relationInsert.do" method="post">
            <div style="display: block; width: 100%; height: 300px;">
                        ...
                        <table width="100%" cellspacing="0" cellpadding="2" border="0" class="toolbg">
                            <tbody>
                                <tr>
                                    <td>
                                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;帐号：
                                        <input type="text" name="accountName" value="${accountName }" maxlength="30"
                    ...
                    <div style="background-color: #c1d9f3;">
                        <table>
                            <c:if test="${ empty list}">
                                <!-- 没有数据  maojdupdate:这个哪张表的没有数据可以省略。只留下面账号处一个即可-->
                                <!-- <div id="tips_bar">
                                    <div class="nomail">
                                        <b>
                                            当前没有数据
                                        ...
                                        <div class="u">
                                            <ul class="scrol">
                                                <c:forEach items="${list }" var="li">
                                                    <li title="${li.cateDesc }" targetid="${li.ta_id }">
                                                        <a style="height: 32px; width: 135px; display: block;" href="#" onclick="return chooseTa_id('${li.ta_id}')">
                        ...
                        <table id="contactTable" cellspacing="0" cellpadding="0" style="table-layout: fixed; width: 100%; *width: auto; background-color: white;">
                            <tbody>
                                ...
                                <c:if test="${not empty relation }">
                                    <c:forEach items="${relation }" var="rel" varStatus="vs">
                                        <tr style="height: 20px;">
                                            <td class="o_title2" width="5%">
                                                <input type="checkbox" name="chkbox" value="${rel.accountName }" title=""/>
                                            </td>
                                            <td width="5%" style="padding-left: 10px;">
                                                ${vs.index+1 }
                                            </td>
                                            <td width="35%"  style="padding-left: 10px;">
                                                <a href="#" onclick="return onFind('${rel.accountName }')" style="text-decoration: underline">
                                                    ${rel.accountName }
                                                </a>
                                            </td>
                                            <td width="35%"  style="padding-left: 10px;">
                                            ...
                                            <td width="20%" style="padding-left: 10px;">
                                                <c:if test="${delPrivlg  || wholePrivlg}">
	                                                <a onclick="return onDelete('${rel.accountName }')" title="删除" href="#" style="text-decoration: underline">
	                                                    删除
	                                                </a>
												</c:if>
												<c:if test="${updatePrivlg || wholePrivlg}">
	                                                &nbsp;
	                                                <a onclick="return onUpdate('${rel.accountName }')" title="修改" href="#" style="text-decoration: underline">
	                                                    修改
	                                                </a>
												</c:if>
                                            </td>
     		...
     		<adtec:page url="relationList.do" totalLines="${pageModel.count }" curpage="${pageModel.pageNow }" pagesize="${pageModel.pageSize }"></adtec:page>
...
<!-- 下面是树状结构 -->
	...
	<script>
		...
		function onFind(obj)
		{
		   ...
		   {
		      action="<%=request.getContextPath()%>/relation/relationFindById.do?accountName="+obj
		      submit();
		//    alert(action);
		   }
		}
	    ...
	    function onSelect(obj)
	    {
	       ...
	       with(document.forms[0])
	       {
	    	   action="<%=request.getContextPath()%>/relation/relationList.do?pageNow="+obj;
	          submit();
	       }
	    }
	    ...
	    function onUpdate(obj)
	    {
	       with(document.forms[0])
	       {
	          action="<%=request.getContextPath()%>/relation/relationUpdate.do?accountName="+obj
		          submit();
//		          alert(obj);
		  //        alert(action);
		       }
	    ...
	    //通过ta_id选择表
		    function chooseTa_id(obj){
		        with(document.forms[0])
		        {
		    //	document.getElementById("cateName").value=obj;
		    	//alert(1);
		    	action="<%=request.getContextPath()%>/relation/relationList.do?ta_id="+obj
		    			submit();
		    	}
		    }
...
//					alert($("#ta_id").val());
					...
					window.parent.adtec_confirm.confirm("确认删除",function (a){
						if(a==1){
						 	 $.ajax({
								   type:"POST",
								   async : false,
								   url:"${pageContext.request.contextPath}/relation/batchdelete.do",
								   data:{"accountName":chkstr,"ta_id":$("#ta_id").val()},
								   success: function(msg){
									//   alert(msg);
									//$("#msg").html("删除成功");
									//showMsg();
			...
			window.parent.adtec_confirm.confirm("确认删除",function (a){
				if (a == 1){
					$.ajax({
								type : "POST",
								async : false,
								url : "${pageContext.request.contextPath}/relation/delAction.do",
								dataType : "json",
		...
		$(function() {
			...
			//计算显示位置
			$(".tab_title ul li[targetid='" + ta_idAfter + "']").addClass(
					"selected");
			var obj = $(".tab_title ul li[targetid='" + ta_idAfter + "']");
